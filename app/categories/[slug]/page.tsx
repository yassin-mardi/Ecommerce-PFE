...
export default function CategoryPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    ...
    // Filter products by category - using exact category name from the category object
    let categoryProducts = products.filter((product) => {
      console.log(`Comparing: "${product.category}" with "${category.name}"`)
    ...
    if (minPrice !== "") {
      categoryProducts = categoryProducts.filter((product) => product.price >= Number.parseFloat(minPrice))
    }
    if (maxPrice !== "") {
      categoryProducts = categoryProducts.filter((product) => product.price <= Number.parseFloat(maxPrice))
    }
  ...
  }, [category, minPrice, maxPrice, inStockOnly, outOfStockOnly, sortOption])
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="grid gap-1">
                      ...
                      <input
                        ...
                        className="w-full h-8 rounded-md border border-input bg-background px-2 text-sm hover:border-primary focus:border-primary transition-colors"
                      />
                    </div>
                    <div className="grid gap-1">
                      ...
                      <input
                        ...
                        className="w-full h8 rounded-md border border-input bg-background px-2 text-sm hover:border-primary focus:border-primary transition-colors"
                      />
                    </div>
                  </div>
                </div>
